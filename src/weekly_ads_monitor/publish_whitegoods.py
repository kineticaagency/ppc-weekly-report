...
from .calltouch import fetch_yandex_direct_leads_by_session
...
def main() -> None:
    ...
    parser.add_argument("--config", default="config/whitegoods.json")
    ...
    calltouch = config["calltouch"]
    leads = fetch_yandex_direct_leads_by_session(
        secrets["CALLTOUCH_API_TOKEN"], calltouch["site_id"], whole,
        calltouch.get("attribution", 1),
    )
    for day, count in leads.items():
